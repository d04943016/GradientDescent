...
def MsgPre(diff, a_new, f=None, varStrList=None, varList=None, MSGOPENBOOL=False):
    # Information
    countList = [0]
    diffList = [diff]
    a_history = [a_new]
    LossList = [] if f==None else [f(a_new)]
    if not MSGOPENBOOL:
    ...
    if f==None:
        if (len(a_new)<=_DATA_LEN_):
            ...
            print('='*25)
            print('{0:>6d} {1:>15.6e} {2}'.format(0, diff, a_new))
        else:
            print('{0:>6s} {1:>15s}'.format('Iter.', 'da_diff'))
            print('='*22)
            print('{0:>6d} {1:>15.6e}'.format(0, diff))
    else:
        if (len(a_new)<=_DATA_LEN_):
            print('{0:>6s} {1:>15s} {2:>15s} {3}'.format('Iter.', 'Loss', 'da_diff', 'a'))
            print('='*40)
            print('{0:>6d} {1:>15.6e} {2:>15.6e} {3}'.format(0, LossList[-1], diff, a_new))
        else:
            print('{0:>6s} {1:>15s} {2:>15s}'.format('Iter.', 'Loss', 'da_diff'))
            print('='*40)
            print('{0:>6d} {1:>15.6e} {2:>15.6e}'.format(0, LossList[-1], diff))
    return countList, diffList, a_history, LossList
def MsgRunning(Info, count, diff, a_new, f=None, MSGCount=100, MSGOPENBOOL=False):
    countList, diffList, a_history, LossList = Info
    # Information
    countList += [count] 
    diffList  += [diff]
    a_history += [a_new]
    LossList = LossList if f==None else LossList+[f(a_new)]
    if not MSGOPENBOOL:
        return countList, diffList, a_history, LossList
    # Msg Box Title
    if (count%MSGCount==0):
    ...
    return countList, diffList, a_history, LossList
def MsgPost(Info, count, diff, a_new, f=None, MSGOPENBOOL=False):
    countList, diffList, a_history, LossList = Info
    # Information
    countList += [count] 
    diffList  += [diff]
...
def ResultsSavor(a_new, count, countList, diffList, a_history, LossList, savefilepath, savefilename):
    ...
    return Appendix
def StochasticGradientDescent(dfda, a_init, learning_rate=0.01, f=None, tol=1e-5, iter_TOL=5e4, MSGOPENBOOL=True, difffun=diffFun, MSGCount=100):
    # initialize
    ...
    Info = MsgPre(diff, a_new, f=f, varStrList=varStrList, varList=varList, MSGOPENBOOL=MSGOPENBOOL)
    # for loop
    while ( (count<iter_TOL) and diff>tol):
        # algorithm
        a_old = a_new.copy()
        da = learning_rate * np.array(dfda(a_old), dtype=np.float64)
        a_new = a_old - da
        diff = difffun(a_new, a_old)
        count += 1
        # Information 
        Info = MsgRunning(Info, count, diff, a_new, f=f, MSGCount=MSGCount, MSGOPENBOOL=MSGOPENBOOL)
    # Information
    countList, diffList, a_history, LossList = MsgPost(Info, count, diff, a_new, f=f, MSGOPENBOOL=MSGOPENBOOL)
    return a_new, count, countList, diffList, a_history, LossList
def Momentum(dfda, a_init, v_init=0.0, learning_rate=0.001, beta=0.9, f=None, tol=1e-5, iter_TOL=5e4, MSGOPENBOOL=True, difffun=diffFun, MSGCount=100):
    # initialize
    a_init = np.array(a_init, dtype=np.float64)
    learning_rate = np.abs(learning_rate)
    count = 0
    ...
    y = None if f==None else f(a_new)
    Info = MsgPre(diff, a_new, f=f, varStrList=varStrList, varList=varList, MSGOPENBOOL=MSGOPENBOOL)
    # for loop
    while ( (count<iter_TOL) and diff>tol):
        # algorithm
        a_old = a_new.copy()
        vt = beta*vt - learning_rate * np.array(dfda(a_old), dtype=np.float64)
        a_new = a_old + vt
        diff = difffun(a_new, a_old)
        count += 1
        # Information 
    ...
    countList, diffList, a_history, LossList = MsgPost(Info, count, diff, a_new, f=f, MSGOPENBOOL=MSGOPENBOOL)
    return a_new, count, countList, diffList, a_history, LossList
def AdaGrad(dfda, a_init, epsilon=1e-8, learning_rate=0.5, f=None, tol=1e-5, iter_TOL=5e4, MSGOPENBOOL=True, difffun=diffFun, MSGCount=100):
    # initialize
    a_init = np.array(a_init, dtype=np.float64)
    learning_rate = np.abs(learning_rate)
    count = 0
    ...
    while ( (count<iter_TOL) and diff>tol):
        # algorithm
        a_old = a_new.copy()
        grad = np.array(dfda(a_old), dtype=np.float64)
        sigma = sigma+grad**2
    ...
    countList, diffList, a_history, LossList = MsgPost(Info, count, diff, a_new, f=f, MSGOPENBOOL=MSGOPENBOOL)
    return a_new, count, countList, diffList, a_history, LossList
def AdaDelta(dfda, a_init, rho=0.9, epsilon=1e-8, f=None, tol=1e-5, iter_TOL=5e4, MSGOPENBOOL=True, difffun=diffFun, MSGCount=100):    # initialize
    a_init = np.array(a_init, dtype=np.float64)
    count = 0
    diff = np.inf
    a_new = a_init
    ...
    countList, diffList, a_history, LossList = MsgPost(Info, count, diff, a_new, f=f, MSGOPENBOOL=MSGOPENBOOL)
    return a_new, count, countList, diffList, a_history, LossList
def RMSprop(dfda, a_init, rho=0.9, epsilon=1e-8, learning_rate=0.01, f=None, tol=1e-5, iter_TOL=5e4, MSGOPENBOOL=True, difffun=diffFun, MSGCount=100):
    # initialize
    a_init = np.array(a_init, dtype=np.float64)
    learning_rate = np.abs(learning_rate)
    count = 0
    ...
    countList, diffList, a_history, LossList = MsgPost(Info, count, diff, a_new, f=f, MSGOPENBOOL=MSGOPENBOOL)
    return a_new, count, countList, diffList, a_history, LossList
def Adam(dfda, a_init, beta1=0.9, beta2=0.999, epsilon=1e-8, learning_rate=0.1, f=None, tol=1e-5, iter_TOL=5e4, MSGOPENBOOL=True, difffun=diffFun, MSGCount=100):
    # initialize
    a_init = np.array(a_init, dtype=np.float64)
    learning_rate = np.abs(learning_rate)
    count = 0
...
if __name__ == '__main__':
    ...
    f = lambda x: (x[0]-2)**2/2 + (x[1]-2)**4/4
    dfda = lambda x: np.array( [(x[0]-2)**1, (x[1]-4)**3], dtype=np.float64 )
    MSGOPENBOOL = False
    tol = 1e-5
    a_init = [0.0,0.0]
    
    print( 'StochasticGradientDescent' )
    a_new, count, countList, diffList, a_history, LossList = StochasticGradientDescent(dfda, a_init=[0,0], tol=tol, f=f, MSGOPENBOOL=MSGOPENBOOL)
    print( a_new, count, '\n' )
   
    print( 'Momentum' )
    a_new, count, countList, diffList, a_history, LossList = Momentum(dfda, a_init=a_init, tol=tol, f=f, MSGOPENBOOL=MSGOPENBOOL)
    print( a_new, count, '\n' )
    
    print( 'AdaGrad' )
    a_new, count, countList, diffList, a_history, LossList = AdaGrad(dfda, a_init=a_init, tol=tol, f=f, MSGOPENBOOL=MSGOPENBOOL)
    print( a_new, count, '\n' )
    
    print( 'AdaDelta' )
    a_new, count, countList, diffList, a_history, LossList = AdaDelta(dfda, a_init=a_init, tol=tol, f=f, MSGOPENBOOL=MSGOPENBOOL)
    print( a_new, count, '\n' )
    
    print( 'RMSprop' )
    a_new, count, countList, diffList, a_history, LossList = RMSprop(dfda, a_init=a_init, tol=tol, f=f, MSGOPENBOOL=MSGOPENBOOL)
    print( a_new, count, '\n' )
   
    print( 'Adam' )
    a_new, count, countList, diffList, a_history, LossList = Adam(dfda, a_init=a_init, tol=tol, f=f, MSGOPENBOOL=MSGOPENBOOL)
    print( a_new, count, '\n' )
